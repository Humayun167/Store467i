...
export function PromptUnlockModal({ product, onClose }: PromptUnlockModalProps) {
  ...
  // Fallback if no promptData is defined (generic digital product)
  const isPromptProduct = !!prompt;
                ...
                <div className="space-y-6">
                  ...
                  <div className="rounded-2xl border border-primary/30 bg-[image:var(--gradient-glow)] p-6 text-center">
                    ...
                    <button
                      ...
                      className="glow-ring w-full flex items-center justify-center gap-2 rounded-full bg-[image:var(--gradient-brand)] py-3.5 text-sm font-semibold text-primary-foreground shadow-[var(--shadow-glow)] transition-transform hover:scale-[1.02] active:scale-[0.98] disabled:opacity-70"
                    >
                      {isUnlocking ? (
                        <>Processing Payment...</>
                      ) : (
